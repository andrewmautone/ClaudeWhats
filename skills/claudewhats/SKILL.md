...
O comando `claudewhats` vem do `bin/` deste plugin e baixa o binário da release na primeira execução (fica em `~/.claudewhats/bin/`). Se o usuário nunca parou, ou qualquer comando falhar com "não pareado", diga para ele rodar `claudewhats serve` no próprio terminal e escanear o QR.
...
3. Se um comando erra com "não pareado", diga ao usuário para rodar `claudewhats serve` uma vez e escanear o QR.
