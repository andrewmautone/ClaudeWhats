...
9. O usuário pode querer as mensagens **cruas** de um período: entregue-as como estão (sem resumir) — coladas no chat quando couberem, ou com `--out <arquivo>` quando ele pedir para salvar/exportar ou quando for muito (centenas de mensagens); diga o caminho. Só resuma quando ele pedir resumo. Períodos: `--since`/`--until` aceitam `24h`, `7d` ou datas `2026-09-01`; para "de X até Y dias atrás" use `--since Yd --until Xd`.
